      ...
      for (const det of dto.detalles) {
        ...
        if (!updated) {
          throw new ConflictException(
            `Stock insuficiente en origen para producto ${det.productoId}`,
          );
        }
    ...
    return await this.db.sql.begin(async (sql: any) => {
      ...
      if (!updated) {
        throw new BadRequestException(
          'Traslado no válido para recepción o ya fue recibido previamente',
        );
      }

      const detalles =
        await sql`SELECT * FROM gym.traslado_detalle WHERE traslado_id = ${trasladoId}`;
