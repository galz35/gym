...
export class MembresiasService {
    ...
    async create(empresaId: string, dto: any) {
        return await this.db.sql.begin(async (sql: any) => {
            const [plan] = await sql`SELECT * FROM gym.plan_membresia WHERE id = ${dto.plan_id}`;
            ...
            const [cliente] = await sql`SELECT nombre FROM gym.cliente WHERE id = ${dto.cliente_id}`;
    ...
    async renovar(id: string, dto: any) {
        return await this.db.sql.begin(async (sql: any) => {
            ...
            const [plan] = await sql`SELECT * FROM gym.plan_membresia WHERE id = ${dto.plan_id || anterior.plan_id}`;
            if (!plan) throw new NotFoundException('Plan no encontrado');
            ...
            const [cliente] = await sql`SELECT nombre FROM gym.cliente WHERE id = ${anterior.cliente_id}`;
