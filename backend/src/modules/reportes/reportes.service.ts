...
export class ReportesService {
    ...
    async getResumenDia(empresaId: string, sucursalId: string, fecha: Date) {
        ...
        const [
            ...
            this.db.sql`SELECT count(*)::integer FROM gym.asistencia WHERE empresa_id = ${empresaId} AND sucursal_id = ${sucursalId} AND fecha_hora >= ${inicio} AND fecha_hora <= ${fin}`,
            this.db.sql`SELECT count(*)::integer FROM gym.asistencia WHERE empresa_id = ${empresaId} AND sucursal_id = ${sucursalId} AND fecha_salida >= ${inicio} AND fecha_salida <= ${fin}`,
            this.db.sql`SELECT count(*)::integer as cant, sum(total_centavos)::bigint as total FROM gym.venta WHERE empresa_id = ${empresaId} AND sucursal_id = ${sucursalId} AND creado_at >= ${inicio} AND creado_at <= ${fin} AND estado = 'APLICADA'`,
            this.db.sql`SELECT sum(monto_centavos)::bigint as total FROM gym.pago WHERE empresa_id = ${empresaId} AND sucursal_id = ${sucursalId} AND creado_at >= ${inicio} AND creado_at <= ${fin} AND estado = 'APLICADO'`,
            this.db.sql`SELECT count(*)::integer FROM gym.cliente WHERE empresa_id = ${empresaId} AND creado_at >= ${inicio} AND creado_at <= ${fin}`
        ]);
