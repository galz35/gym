...
export class PlanesService {
  ...
  async findOne(id: string) {
    const [plan] = await this.db
      .sql`SELECT * FROM gym.plan_membresia WHERE id = ${id}`;
    if (!plan) throw new NotFoundException('Plan no encontrado');
    return plan;
  }

  async create(empresaId: string, dto: CreatePlanDto) {
    if (dto.tipo === 'DIAS' && !dto.dias)
      throw new BadRequestException('Debe especificar días para tipo DIAS');
    if (dto.tipo === 'VISITAS' && !dto.visitas)
      throw new BadRequestException(
        'Debe especificar visitas para tipo VISITAS',
      );
  ...
  async update(id: string, dto: UpdatePlanDto) {
    ...
    if (dto.precio !== undefined)
      updates.precio_centavos = Math.round(dto.precio * 100);
